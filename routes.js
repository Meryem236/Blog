    ...
    function reqListener() {
        if (this.status === 200) {
            ...
            Promise.all(promises).then(updatedArticles => {
                ...
                // After rendering the articles, attach click event listeners to article links
                document.querySelectorAll('.article-link').forEach(link => {
                    link.addEventListener('click', (event) => {
                        event.preventDefault();
                        const articleId = event.target.dataset.articleId;
                        const offset = event.target.dataset.offset;
                        const totalCount = event.target.dataset.totalCount;
...
function addArtDetailLink2ResponseJson(responseJSON) {
    responseJSON.articles = responseJSON.articles.map(article => ({
        ...article,
        detailLink: `#article/${article.id}/0/0` // Adjust the offset and totalCount as needed
    }));
}                                    
    ...
    fetch(url)
        ...
        .then(article => {
            ...
            const articleData = {
                ...
                editLink: `#artEdit/${article.id}/${offsetFromHash}/${totalCountFromHash}`,
                deleteLink: `#artDelete/${article.id}/${offsetFromHash}/${totalCountFromHash}`
            };

            // Render the article details using Mustache
            document.getElementById(targetElm).innerHTML = Mustache.render(
...
function handleAddCommentFormSubmit(event, artIdFromHash) {
    ...
    const form = event.target; // Access the submitted form
